--in this version, neither of the inputs from input.conf work :D
...
local function time_remaining()
    return function()
        ...
        elseif toggle == true then
            mp.commandv("cycle-values", "osd-msg1", printout, "")
            on_screen = not on_screen
            if on_screen == true then msg.info(printout) end
        end
    end
end

mp.add_key_binding('Alt+p', 'time-rem', time_remaining())
mp.add_key_binding('Alt+Shift+p', 'time-rem-toggle', time_remaining())
