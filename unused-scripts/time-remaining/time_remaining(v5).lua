--this one actually works with keybinds! but only specifically in the form
--Alt+w script-binding time_remaining/time-rem
--Alt+Shift+w script-binding time_remaining/time-rem-toggle
--NOT
-- Alt+w script-message-to time_remaining time-rem
-- Alt+Shift+w script-message-to time_remaining time-rem-toggle
--ALSO
--for some reason it has to be 
...
--???? weird asf
--idk the last part maybe isn't, like maybe they each have their own preferred input associated
--but that doesn't make sense bc i was still getting input with the other one.. and its not like 
--the other one worked nice with its own either. shrug kaomoji
...
mp.add_key_binding('Alt+p', 'time-rem', time_remaining(false))
mp.add_key_binding('Alt+Shift+p', 'time-rem-toggle', time_remaining(true))
-- mp.register_script_message('tr', time_remaining(false))
-- mp.register_script_message('trt', time_remaining(true))